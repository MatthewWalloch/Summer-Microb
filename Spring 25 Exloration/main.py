...
def main_QS(init_SN, sig_Cost ,lam , K, Auto=False, max_G=500, clonal=True):
    ...
    # eval_genotype
    eval_genotype_No_Auto_Clonal(fit_Pop,coopPayoff_Pop,coopCost_Pop,sigCost_Pop,
    ...
    for g in range(1,max_G):
        ...
        pro_Rate = mut_parameter(pro_Rate,mu_Production,mu_SD_ProRate,min_ProRate,max_ProRate,size_Pop)
        # mutate signal threshold
        sensitivity = mut_parameter(sensitivity,mu_sensitiity,mu_SD_sensitivity,min_sensitivity,max_sensitivity,size_Pop)
        # mutate ratio of autoinduction production
        auto_R = mut_parameter(auto_R,mu_R,mu_SD_R,min_R,max_R,size_Pop)
        ...
        if g % 500 == 499:
            ...
            # file = f"New python\json\log mult sense 10k\gen {g+1}.json"
    ...
    # else:
    #     file = f"New python\json\log many geno\\{timestr} {lam} {init_SN} {sig_Cost} {max_G} {clonal}.json"
...
def vary_genotype(lam, Auto):
    clonal = False
    # t = time.time_ns()
    init_SN, sig_Cost, K, mu_Cheats, max_G = 1, 10**9, 50.0, 10.0 ** -4, 5000
...
if __name__ == "__main__":
    ...
    clonal = False
    # # t = time.time_ns()
    init_SN, sig_Cost, lam, K, max_G = 1, 10**9, 10, 50.0, 5000
    file = main_QS(init_SN, sig_Cost ,lam , K, max_G=max_G, Auto=Auto, clonal=clonal)
