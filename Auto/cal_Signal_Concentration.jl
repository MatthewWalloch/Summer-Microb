...
function cal_Signal_Concentration(p::Matrix{Float64}, N::Matrix{Float64}, u::Float64, r::Matrix{Float64}, K::Float64)
	print(((N.^2 .*p.^2 .*r.^2 + 2 .*N.^2 .*p.^2 .*r + N.^2 .*p.^2 - 2*K .*N.*p.*r*u + 2*K .*N.*p*u .+ K^2 *u^2).^(1/2) + N.*p .- K*u + N.*p.*r)/(2 *u)	)
	return ((N.^2 .*p.^2 .*r.^2 + 2 .*N.^2 .*p.^2 .*r + N.^2 .*p.^2 - 2*K .*N.*p.*r*u + 2*K .*N.*p*u .+ K^2 *u^2).^(1/2) + N.*p .- K*u + N.*p.*r)/(2 *u)
